#include <pthread.h>
#include "libmcount/mongoose.h"
#include "libmcount/frozen.h"
...
static void handle_list(struct mg_connection *nc, struct http_message *hm) {
	...
	list_for_each_entry(pos, &ifs, list) {
		mg_printf_http_chunk(nc, "{\"name\":\"%s\", \"active\":%s}%s",
				     pos->sym->name, pos->active ? "true" : "false",
...
static void handle_set(struct mg_connection *nc, struct http_message *hm) {
	...
	symtabs = ((struct server_data*)nc->user_data)->symtabs;
	for (i = 0; json_scanf_array_elem(hm->body.p, hm->body.len,
					  ".functions", i, &t) > 0; i++) {
		// for every function listed, set its status
		int active = 1;
		char *function = NULL;

		json_scanf(t.ptr, t.len, "{ name:%Q, active:%B }", &function, &active);
		if (!function)
			continue;
		...
		if (active)
			enable_sym_dynamic(s);
		else
			disable_sym_dynamic(s);
	}
